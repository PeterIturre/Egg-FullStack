...
SELECT nombre, sal_emp, comision_emp, id_depto FROM empleados WHERE id_depto = 2000;

/*12. Obtener el valor total a pagar a cada empleado del departamento 3000, que resulta
de: sumar el salario y la comisión, más una bonificación de 500. Mostrar el nombre del
empleado y el total a pagar, en orden alfabético.*/
SELECT nombre, (sal_emp + comision_emp + 500) AS 'Total', id_depto FROM empleados WHERE id_depto = 3000 ORDER BY nombre ASC;
...
SELECT DISTINCT nombre, id_depto, sal_emp FROM empleados WHERE sal_emp > (SELECT AVG(sal_emp) FROM empleados);
SELECT nombre, id_depto, sal_emp FROM empleados WHERE sal_emp > (SELECT AVG(sal_emp) FROM empleados) ORDER BY id_depto ASC; 
